...
```bash
Grafana（展示端） → 连接 → Prometheus（数据源） → 连接 → 你的Go程序
```

#### 1.1 添加数据源（关键步骤）

- **目的**​：告诉Grafana去哪里获取监控数据

**操作步骤**
- 打开Grafana：http://localhost:3000 (默认账号admin/admin)

---
> 第 1～3 步，如下图，找到添加数据源的地方（Data sources 可以看见已配置的数据源）

![GrafanaAddConnData01](./image/GrafanaAddConnData01.png)

---

> 第 4 步：选择数据源 Prometheus

![GrafanaAddConnData02](./image/GrafanaAddConnData02.png)

---

> 第 5 步：配置核心参数 与 保存（点击 ​Save & Test）

![GrafanaAddConnData03](./image/GrafanaAddConnData03.png)
...
![CreateDashboard01](./image/CreateDashboard01.png)
...
![CreateDashboard02](./image/CreateDashboard02.png)
...
|数据类型	|推荐图表	|案例说明|
|:----|:----|:----|
|实时变化值	|折线图(Time series)	|CPU温度波动|
|当前状态值	|仪表盘(Gauge)	|内存使用百分比|
|分布统计	|柱状图(Bar chart)	|请求延迟分布|
|文本信息	|表格(Table)	|服务版本列表|
...
![Dashboard01](./image/Dashboard01.png)
